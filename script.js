  ...
  try {
    ...
    if (saved) {
      return {
        ...
        topics: saved.topics || defaultTopics,
...
function render() {
  ...
  if (currentView === "topic-detail") root.innerHTML = topicDetailView(currentTopicIndex);
...
function pageHeader(title, subtitle, buttonText = "") {
  return `
    <div class="section-heading">
      ...
      ${buttonText ? `<button class="write-btn" id="pageAction">${escapeHTML(buttonText)}</button>` : ""}
    </div>
  `;
}
...
function entryRow(entry) {
  ...
  return `
    <button class="entry" data-entry="${entry.id}" style="width:100%;background:none;border:0;text-align:left">
...
function topicsView() {
  return `
    <div class="section-heading">
      ...
      <button class="write-btn" id="newTopicBtn">+ New topic</button>
    </div>
        ...
        ? state.topics.map((topic, index) => `
          <article class="topic-card">
            ...
            <div class="topic-actions">
              ...
              <button class="topic-delete" data-delete-topic="${index}">×</button>
            </div>
          </article>
        `).join("")
        : emptyState("No topics yet.", "Create a topic to start collecting thoughts.")}
    </div>
...
function topicDetailView(index) {
  ...
  return `
    <div class="section-heading">
      <div>
        <button class="back" id="backToTopics">← Topics</button>
        <h1 class="page-title">${escapeHTML(topic)}</h1>
        <p class="page-subtitle">Thoughts and writing connected to this topic.</p>
      </div>

      <button class="write-btn" id="addArticleBtn">+ Add article</button>
    </div>
    ...
    ${articles.length
      ? `
        <div class="entries">
          ${articles.map(article => `
            <button class="entry" data-article="${article.id}" style="width:100%;background:none;border:0;text-align:left">
              <div class="date">${formatDate(article.date)}</div>
              <div>
                <h3 class="entry-title">${escapeHTML(article.title || "Untitled article")}</h3>
                <p class="entry-preview">${escapeHTML(article.content.replace(/\s+/g, " ").slice(0, 150))}</p>
              </div>
      ...
      `
      : emptyState("Nothing here yet.", "Start writing something about this topic.")}
  `;
}
...
function createTopicModal() {
  ...
  modal.innerHTML = `
    <div class="modal-backdrop" data-close-topic></div>

    <div class="modal-card">
      <div class="modal-top">
        <div class="eyebrow">NEW TOPIC</div>
        <button class="modal-close" data-close-topic type="button">×</button>
      </div>
      ...
      <p class="page-subtitle" style="margin-top:24px">
        A subject you want to keep coming back to.
      </p>

      <div class="modal-footer" style="display:flex;justify-content:flex-end;gap:24px">
        <button class="text-link" data-close-topic type="button">Cancel</button>
        <button class="write-btn" id="saveTopicBtn" type="button">Create topic</button>
      </div>
    </div>
  `;
  ...
  modal.querySelectorAll("[data-close-topic]").forEach(btn => {
    btn.addEventListener("click", closeTopicModal);
  });

  document.getElementById("saveTopicBtn").addEventListener("click", saveTopic);

  document.getElementById("topicName").addEventListener("keydown", e => {
...
function saveTopic() {
  ...
  if (state.topics.some(topic => topic.toLowerCase() === name.toLowerCase())) {
...
function editTopic(index) {
  ...
  if (state.topics.some((item, i) =>
    item.toLowerCase() === topic.toLowerCase() && i !== index
  )) {
    alert("This topic already exists.");
    return;
  }
  ...
  if (state.articles) {
    state.articles.forEach(article => {
      if (article.topic === oldName) article.topic = topic;
    });
  }
...
function createArticleModal() {
  ...
  modal.innerHTML = `
    <div class="modal-backdrop" data-close-article></div>

    <div class="modal-card">
      <div class="modal-top">
        <div class="eyebrow">NEW ARTICLE</div>
        <button class="modal-close" data-close-article type="button">×</button>
      </div>
      ...
      <div class="modal-footer">
        <button class="write-btn" id="saveArticleBtn" type="button">
          Save article
        </button>
      </div>
    </div>
  ...
  modal.querySelectorAll("[data-close-article]").forEach(btn => {
...
function openArticleModal() {
  ...
  setTimeout(() => document.getElementById("articleTitle").focus(), 50);
}
...
function showArticle(id) {
  ...
  document.getElementById("app").innerHTML = `
    <article class="reading">
      <button class="back" id="backArticleBtn">← ${escapeHTML(article.topic)}</button>
      <div class="eyebrow">${escapeHTML(article.topic)}</div>
  ...
  document.getElementById("backArticleBtn").addEventListener("click", render);
}
...
function bindViewEvents() {
  ...
  document.querySelectorAll("[data-article]").forEach(button => {
    button.addEventListener("click", () => showArticle(button.dataset.article));
  });
  ...
  if (newTopicBtn) newTopicBtn.addEventListener("click", addTopic);

  document.querySelectorAll("[data-open-topic]").forEach(button => {
    button.addEventListener("click", () => {
      currentTopicIndex = Number(button.dataset.openTopic);
  ...
  if (addArticleBtn) addArticleBtn.addEventListener("click", openArticleModal);
}
...
document.addEventListener("keydown", e => {
  ...
  if (thoughtModal && !thoughtModal.classList.contains("hidden")) {
    closeModal();
  }
});
